...
ZIPを任意の書き込み可能な場所へ展開し、展開先の`NicoCache_nl.cmd`を起動する。この入口は
同梱`jre/bin/javaw.exe`から`NicoCacheLauncher.jar`を実行する。初回セットアップはMSIと同じで
ある。`Program Files`のような保護された場所へZIPを展開して直接編集せず、個人のフォルダーを
使用すること。
...
本体の起動中は別プロセスの診断アプリが監視する。ランチャーの「本体を停止」を使うと本体と
診断アプリが正常終了し、ランチャーだけが残る。
