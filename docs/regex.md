...
### 上級：現行CMAFキャッシュディレクトリの簡易判定

```
^[a-z]{2}\d{1,12}[^/\\]*\.hls$
```

動画ID、同じパス要素内の付加情報、現行Domand/CMAFキャッシュの`.hls`末尾を確認する簡易例で
ある。キャッシュ名の`low`は保存済み旧形式との互換用であり、現行品質の判定には使わない。
映像モード、音声ビットレート、優先キャッシュはファイル名から推測せず、
[`/cache/info/v3`](https://github.com/roflsunriz/NicoCache_nl/blob/main/documents/api.md)の
構造化された値を使用する。
